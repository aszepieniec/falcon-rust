...
impl<const N: usize> PublicKey<N> {
    ...
    pub fn from_bytes(byte_array: &[u8]) -> Result<Self, FalconDeserializationError> {
        ...
        Ok(PublicKey { h_ntt: h.fft() })
    }

    // Serialize the public key as a list of bytes.
    pub fn to_bytes(&self) -> Vec<u8> {
        let h = self.h_ntt.ifft();
...
pub fn verify<const N: usize>(m: &[u8], sig: &Signature<N>, pk: &PublicKey<N>) -> bool {
    ...
    let mut s2_norm_sq = 0i64;
    let s2_felt: Vec<Felt> = s2
        .iter()
        .map(|&a| {
            s2_norm_sq += (a as i64) * (a as i64);
            Felt::from(a)
        })
        .collect_vec();
    ...
    let s1_norm_sq = s1
        ...
        .map(|i| i * i)
        .sum::<i64>();
    let length_squared = s1_norm_sq + s2_norm_sq;
...
mod test {
    ...
    #[test]
    fn test_public_key_serialization_golden() {
        // Pin the byte output for a fixed seed so that any change to the wire
        ...
        #[rustfmt::skip]
        let expected: &[u8] = &[
            9, 168, 149, 47, 4, 41, 201, 43, 115, 93, 73, 138, 86, 40, 72, 155,
            36, 161, 148, 225, 74, 155, 78, 49, 23, 133, 190, 19, 76, 58, 218, 239,
            118, 242, 229, 71, 10, 96, 218, 117, 61, 150, 252, 55, 132, 89, 7, 84,
            195, 10, 136, 226, 171, 118, 2, 29, 21, 67, 10, 129, 5, 15, 137, 118,
            28, 118, 131, 192, 195, 73, 42, 96, 50, 30, 65, 65, 160, 248, 137, 198,
            87, 192, 87, 192, 127, 108, 85, 38, 184, 38, 174, 230, 178, 140, 85, 104,
            236, 76, 59, 150, 225, 48, 152, 156, 207, 234, 122, 25, 18, 165, 217, 26,
            88, 178, 212, 10, 129, 12, 77, 247, 59, 80, 133, 136, 37, 74, 134, 66,
            234, 101, 193, 240, 36, 104, 26, 126, 156, 114, 143, 145, 175, 140, 186, 24,
            74, 244, 152, 34, 75, 8, 217, 163, 116, 90, 14, 49, 196, 190, 26, 243,
            232, 212, 13, 208, 20, 153, 146, 11, 189, 105, 74, 43, 103, 182, 24, 111,
            143, 217, 87, 176, 94, 226, 144, 126, 160, 205, 114, 99, 144, 147, 110, 84,
            148, 169, 140, 64, 7, 188, 230, 155, 248, 27, 193, 150, 48, 24, 57, 145,
            163, 1, 12, 102, 4, 113, 68, 110, 194, 252, 163, 69, 38, 216, 9, 132,
            160, 157, 65, 163, 218, 69, 2, 60, 50, 81, 198, 147, 57, 39, 231, 82,
            52, 237, 231, 156, 140, 4, 124, 145, 115, 149, 153, 192, 221, 77, 65, 145,
            169, 125, 10, 166, 122, 246, 170, 74, 49, 153, 161, 149, 162, 24, 247, 205,
            224, 71, 121, 69, 12, 41, 222, 130, 133, 152, 246, 160, 122, 28, 170, 138,
            58, 196, 223, 117, 54, 153, 161, 14, 78, 113, 8, 178, 41, 63, 137, 49,
            177, 136, 235, 27, 151, 137, 208, 57, 179, 250, 67, 186, 31, 56, 11, 80,
            155, 162, 242, 187, 38, 132, 2, 20, 172, 69, 118, 62, 3, 248, 88, 235,
            204, 185, 154, 3, 8, 237, 32, 82, 24, 52, 27, 100, 97, 158, 195, 29,
            168, 252, 162, 211, 232, 221, 186, 125, 147, 64, 2, 5, 16, 66, 45, 239,
